...
class Controller_Manager_Pedido extends Controller_Manager_Containers_Manager {

	public function action_index()
	{            
            HTTP::redirect('manager/pedido/create/');
	}
	...
	//IF USER SUBMITTED FORM
        $this->view = View::factory( 'controller/manager/pedido/create' );
	...
	$ID_PEDIDO = $this->request->param('params');
        ...
        if( $this->request->post() )
        {
                ...
                HTTP::redirect('manager/pedido/select/');
        }                        
  ...
  public function action_select()
	{
		$this->view = View::factory('controller/manager/pedido/select');
  ...
  public function action_aprobar()
	{
		$this->view = View::factory('controller/manager/pedido/aprobar');
                $pedidosp = DB::query(Database::SELECT, "SELECT Distinct(`ID_PEDIDO`) FROM `pedido_despacho_x_producto` WHERE `ESTADO`='PEDIDO' AND `ID_DESPACHO` is NULL")->execute()->as_array();
                $pedido = array();
                foreach ($pedidosp as $value) {
                    $pedidos = ORM::factory('pedido')->where('ID_PEDIDO', '=', $value)->find();
  ...
  public function action_revisar()
	{
	//IF USER SUBMITTED FORM
        $this->view = View::factory( 'controller/manager/pedido/revisar' );
	...
	$ID_PEDIDO = $this->request->param('params');
        ...
        if( $this->request->post() )
        {
                ...
                HTTP::redirect('manager/pedido/aprobar/');
        }                        
                $this->view->set( 'pedido', $pedido );
	} 
}
