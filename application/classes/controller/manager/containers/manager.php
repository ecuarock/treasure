...
class Controller_Manager_Containers_Manager extends Controller_Template {
    
    public $template = 'controller/manager/containers/manager';
	...
	public function before()
	{
		...
		parent::before();
            ...
            if(!Session::instance()->get('treasure_usuario', FALSE) OR Session::instance()->get('treasure_usuario', FALSE)=="" OR $user->ID_PERFIL!=3)
            {
                HTTP::redirect('login');
            }
		...
		if ( $this->auto_render )
  	    {
			...
			$this->template->title = 'Treasure Administrador';
