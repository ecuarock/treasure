...
class Controller_Admin_Despacho extends Controller_Admin_Containers_Admin {

	public function action_index()
	{            
            HTTP::redirect('admin/despacho/create/');
	}
	...
	//IF USER SUBMITTED FORM
        $this->view = View::factory( 'controller/admin/despacho/create' );
	...
	$ID_DESPACHO = $this->request->param('params');
        ...
        if( $this->request->post() )
        {
                ...
                HTTP::redirect('admin/despacho/select/');
        }                        
                $this->view->set( '$despacho', $despacho );
	}	
  ...
  public function action_select()
	{
		$this->view = View::factory('controller/admin/despacho/select');
  ...
  public function action_procesar()
	{
		$this->view = View::factory('controller/admin/despacho/procesar');
                $despachosp = DB::query(Database::SELECT, "SELECT Distinct(`ID_DESPACHO`) FROM `pedido_despacho_x_producto` WHERE `ESTADO` in ('EN PROCESO','COMPRADO','ENVIADO') AND `ID_DESPACHO` is not NULL")->execute()->as_array();
                $despacho = array();
                foreach ($despachosp as $value) {
                    $despachos = ORM::factory('despacho')->where('ID_DESPACHO', '=', $value)->find();
                    $despacho = $despacho + array($despachos->ID_DESPACHO=>$despachos->as_array());
  ...
  public function action_revisar()
	{
	//IF USER SUBMITTED FORM
        $this->view = View::factory( 'controller/admin/despacho/revisar' );
        
        //IF USER SUBMITTED FORM        
	$ID_DESPACHO = $this->request->param('params');
        $pedidos = DB::query(Database::SELECT, "SELECT Distinct(`ID_PEDIDO`) FROM `pedido_despacho_x_producto` WHERE `ESTADO` in ('EN PROCESO','COMPRADO','ENVIADO') AND `ID_DESPACHO` = ".$ID_DESPACHO."")->execute()->as_array();
        $this->view->set( 'pedidos', $pedidos);
        $despacho = ORM::factory('despacho')->where('ID_DESPACHO', '=', $ID_DESPACHO)->find();
        $this->view->set( 'despacho', $despacho );
        $user = ORM::factory('user')->where('ID_USUARIO', '=', $despacho->ID_USUARIO)->find();
        ...
        if( $this->request->post() )
        {
                ...
                {   
                    ...
                    switch ($proceso)
                    {
                        ...
                        case 4:foreach ($pedidoProceso as $value) {
                               ...
                               break;
                        default : HTTP::redirect('admin/main');
                            break;
                    }  
                }
                catch(Exception $e)
                ...
                HTTP::redirect('admin/despacho/procesar/');
        }                        
                $this->view->set( '$despacho', $despacho );
	} 
}
