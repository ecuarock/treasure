...
(function($, window, document) {
	...
	_oExt.oSort = {
		...
		"uk_date-asc": function(a,b) {
            ...
            var timeOrNota = ukDatea[2].split(' ');
            if (timeOrNota.length>1){
                var houra = timeOrNota[0].split(':');
                var x = (ukDatea[2] + ukDatea[1] + timeOrNota[0] + houra[0] + houra[1]) * 1;
            }else{
                var x = (ukDatea[2] + ukDatea[1] + ukDatea[0]) * 1;
            }

            var timeOrNotb = ukDateb[2].split(' ');
            if (timeOrNotb.length>1){
                var hourb = timeOrNotb[0].split(':');
                var y = (ukDateb[2] + ukDateb[1] + timeOrNotb[0] + hourb[0] + hourb[1]) * 1;
            }else{
